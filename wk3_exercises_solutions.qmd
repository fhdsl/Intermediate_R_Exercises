...
```{r}
billboard_longer = pivot_longer(billboard_sm, c("wk1", "wk2", "wk3", "wk4"), names_to = "week", values_to = "billboard_ranking")
```

We can visualize this:

```{r}
ggplot(billboard_longer) + aes(x = week, y = billboard_rank, group = track) + geom_line(alpha = .25)
```
...
```{r}
pivot_wider(billboard_longer, names_from= "week",  values_from = "billboard_ranking")
```
...
```{r}
melanoma_sex = pivot_longer(male_vs_female, c("Age_adjusted_incidence_rate.male", "Age_adjusted_incidence_rate.female"), names_to = "Sex", values_to = "Age_adjusted_incidence_rate")
```
...
```{r}
melanoma_sex$Sex[melanoma_sex$Sex == "Age_adjusted_incidence_rate.female"] = "female"
melanoma_sex$Sex[melanoma_sex$Sex == "Age_adjusted_incidence_rate.male"] = "male"
```

There are some `NA`s in the column "Age_adjusted_incidence_rate". Can you filter these rows out?

```{r}
melanoma_sex = filter(melanoma_sex, !is.na(melanoma_sex$Age_adjusted_incidence_rate))
```

Let's make the plot! I'll get you started…

```{r}
ggplot(melanoma_sex, aes(x = County, y = Age_adjusted_incidence_rate, fill = Sex)) + geom_bar(position="dodge", stat="identity") + theme(axis.text.x = element_text(angle = 90)) + labs(title = "Melanoma", y = "Incidence Rate per 100k people")
```
