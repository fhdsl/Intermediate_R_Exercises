...
```{r}
as.character(as.numeric(some_logicals))
```
...
```{r}
penguins_clean2 = select(penguins_clean, species, body_mass_g)
...
penguins_clean_c = filter(penguins_clean, species == "Chinstrap")
mean(penguins_clean_c$body_mass_g)

penguins_clean_a = filter(penguins_clean, species == "Adelie")
mean(penguins_clean_a$body_mass_g)
```
